...
#		# so that they cannot be modified e.g. all classes except grasslands,
#        # pastures and urban, and within those, the pixels where yatsm detected 
...
#		  --outfile=maskC_$yr".tif" \
#          --calc='"logical_or(logical_or(A == 2, A == 4), A == 3) * (B == -9999)"' \
#          --NoDataValue=3 --type=Byte --co="NBITS=2" --overwrite

        # Get final mask. If we're masking all classes out, calculting the 
        # buffer and then this operation are not necessary....
                
#        qsub -j y -V -N mD_$pt$rw"_"$yr -hold_jid mC_$pt$rw"_"$yr -b y \
#         gdal_calc.py -A maskB_$yr"_2-3.tif" -B maskC_$yr".tif" \
...
#        qsub -j y -V -N sv_$pt$rw"_"$yr -hold_jid mD_$pt$rw"_"$yr -b y \
         gdal_sieve.py -st 10 -8  $pre$yr$suf -mask maskC_$yr".tif" \
          $pre$yr"_sievedsimple".tif
#
...
#        qsub -j y -V -N postsv_$yr -hold_jid sv_$pt$rw"_"$yr -b y \
         gdal_calc.py -A $pre$yr$suf -B $pre$yr"_sievedsimple.tif" \
           --outfile=$pre$yr"_postsievedsimple.tif" \
